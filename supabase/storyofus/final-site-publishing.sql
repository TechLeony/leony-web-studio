...
-- Phase 1B: final-site publishing foundation.
-- This migration is additive. It does not auto-publish, backfill, or deliver existing orders.

alter table public.storyofus_submissions
  add column if not exists final_site_slug text;

create unique index if not exists storyofus_submissions_final_site_slug_unique
on public.storyofus_submissions (final_site_slug)
where final_site_slug is not null;

create index if not exists storyofus_submissions_public_final_site_idx
on public.storyofus_submissions (final_site_slug, status)
where final_site_slug is not null
...
begin
  ...
  final_site_slug := v_slug;
  final_site_url := v_url;
  email_queued := v_email_inserted_count > 0;
  return next;
end;
$$;
