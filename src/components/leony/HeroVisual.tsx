    ...
    <div className="relative w-full aspect-[4/3] md:aspect-[5/4]">
        ...
        <div className="p-5 md:p-6 grid grid-cols-5 gap-4">
          <div className="col-span-2 space-y-3">
            ...
            <div className="mt-4 grid grid-cols-2 gap-2">
              {hv.quickLinks.map((q) => (
                <div key={q} className="rounded-lg border border-border bg-background px-2 py-2 text-[10px] font-medium text-muted-foreground">
                  {q}
                </div>
              ))}
            </div>
          ...
          <div className="col-span-3 rounded-xl bg-gradient-to-br from-navy via-purple to-pink p-4 text-navy-foreground relative overflow-hidden">
            ...
            <div className="mt-1 text-lg font-semibold leading-snug">{hv.webBlockTitle}</div>
      ...
      <div className="absolute -left-2 bottom-2 w-[58%] rounded-2xl bg-card border border-border shadow-xl shadow-navy/10 p-4 animate-float-slow">
        <div className="flex items-center gap-2">
          ...
          <div className="flex-1 min-w-0">
            <div className="text-[11px] font-semibold text-foreground">{hv.adminTitle}</div>
            <div className="text-[10px] text-muted-foreground">{hv.adminSub}</div>
          </div>
      ...
      <div className="absolute right-0 bottom-10 w-[48%] rounded-2xl bg-card border border-border shadow-xl shadow-navy/10 p-4 animate-float-slow [animation-delay:1.5s]">
        <div className="flex items-center gap-2">
          ...
          <div>
            <div className="text-[11px] font-semibold text-foreground">{hv.apptTitle}</div>
            <div className="text-[10px] text-muted-foreground">{hv.apptSub}</div>
          </div>
        </div>
      ...
      <Chip className="absolute -top-3 left-4" icon={<Send className="h-3 w-3" />} label={hv.chips.contactForm} />
      <Chip className="absolute top-10 -right-1" icon={<MessageSquare className="h-3 w-3" />} label={hv.chips.whatsappFlow} />
      <Chip className="absolute bottom-32 -left-3 hidden sm:flex" icon={<Smartphone className="h-3 w-3" />} label={hv.chips.mobile} />
      <Chip className="absolute bottom-0 right-12" icon={<Sparkles className="h-3 w-3" />} label={hv.chips.demos} />
    </div>
  );
}
