...
type DecorStyle = "hearts" | "stars" | "capybara" | "cat" | "dog" | "panda";
type GalleryMode = "polaroid" | "carousel" | "stacked";
type AnimationStyle = "soft-slide" | "fade" | "none";
...
function StoryDemo() {
  const story = demoStoryData;
  const customerStory = null as null | {
    spotify?: Partial<typeof demoStoryData.spotify>;
    voiceNote?: Partial<typeof demoStoryData.voiceNote>;
  };
...
function PhotoPuzzleGame({
  ...
  useEffect(() => {
    return () => {
      snapTimers.current.forEach((timer) => window.clearTimeout(timer));
    };
  }, []);
